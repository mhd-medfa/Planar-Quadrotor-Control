function [t_out, s_out, des_state, wind_disturbances] = simulation_2d(controlhandle, trajhandle)
...
% Get boundary
max_steps = t_total/tstep;
des_state = nan(max_steps,6);
wind_disturbances = nan(max_steps, 2);
rand('seed', 1);
for iter = 1:max_steps
    wind_disturbances(iter, 1:2) = 0.01*params.W.*rand(1,2)+params.W/2;
    dd = trajhandle(tstep*iter,[]);
    des_state(iter,1:2) = dd.pos(1:2)';
    des_state(iter,4:5) = dd.vel(1:2)';
    %des_state(iter,5:6) = dd.acc(1:2)';
    
end
d_state = nan(max_iter,2);
for iter = 1:max_iter
    dd = trajhandle(cstep*iter,[]);
    d_state(iter,1:2) = dd.pos(1:2)';
    d_state(iter,3:4) = dd.vel(1:2)';
    d_state(iter,5:6) = dd.acc(1:2)';
...
for iter = 1:max_iter
  ...
  % Run simulation
  params.W = wind_disturbances(iter, 1:2);
  params.step = iter;
